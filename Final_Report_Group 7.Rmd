...
pacman::p_load(tidyverse, readxl, caret, randomForest, e1071, pROC, ROSE, Metrics)
...
```{r, warning=FALSE,fig.width=4,fig.height=3.5,fig.cap="There are only 197 negative response but 2108 positive response."}
##over all
...
```{r, warning=FALSE,fig.width=7,fig.height=2.5,fig.cap="barplot of race and follow guidelines or not"}
##race
...
```{r, warning=FALSE,fig.width=6,fig.height=3,fig.cap="barplot of gender and follow guidelines or not",fig.align='center'}
##sex
ggplot(data=sali)+
...
Figure 3 shows that there are no big difference between male and female.

```{r, warning=FALSE,fig.width=6,fig.height=3,fig.cap="barplot of AJCC Stage and follow guidelines or not",fig.align='center'}
##AJCC cancer stage
ggplot(data=sali)+
...
In Figure 4, it is unusual that all patients at IVB stage did not follow guidelines.
...
```{r, warning=FALSE,fig.width=5,fig.height=4,fig.cap="ROC cureve without using stratified"}
...
table(pred_forest2, test$follow)
```
```{r, warning=FALSE,fig.width=5,fig.height=4,fig.cap="ROC cureve using stratified"}
...
```{r, warning=FALSE,fig.width=6,fig.height=4,fig.cap="This plot shows the importance of each variabels, and AJCC_7_Stage plays an significant role."}
...
table(pred_forest, test$follow)
table(pred_forest1_2, test$follow)
...
table(pred_forest2, test$follow)
table(pred_forest2_2, test$follow)
rf_roc3 <- roc(test$follow,as.numeric(pred_forest2))
plot(rf_roc3, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),grid.col=c("green", "red"), max.auc.polygon=TRUE,auc.polygon.col="skyblue", print.thres=TRUE,main='random forest with stratified sampling ROC curve')

rf_roc4 <- roc(test$follow,as.numeric(pred_forest2_2))
plot(rf_roc4, print.auc=TRUE, auc.polygon=TRUE, grid=c(0.1, 0.2),grid.col=c("green", "red"), max.auc.polygon=TRUE,auc.polygon.col="skyblue", print.thres=TRUE,main='random forest with stratified sampling, without Race, Gender ROC curve')
```
...
We found some methods on [this website](https://machinelearningmastery.com/bagging-and-random-forest-for-imbalanced-classification/), and the Jupyter notebook .ipynb file is uploaded in the Appendix folder on our github. The performance of these models are close to our random forest model. 
